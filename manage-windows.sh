#!/usr/bin/env bash
# ============================================================================
#  Bitmoji Lab Android · management console · WINDOWS
...
SRC_BOOK_DIR="${SRC_BOOK_DIR:-$HOME/koutoub/app-tier-s-bitmoji-lab}"
KEYSTORE_DIR="${KEYSTORE_DIR:-$HOME/.keys}"
KEYSTORE_FILE="${KEYSTORE_FILE:-$KEYSTORE_DIR/wdiy-upload.keystore}"
APP_ID="${APP_ID:-org.workshopdiy.bitmojilab}"
AVD_NAME="${AVD_NAME:-bitmoji_lab_test}"
...
cmd_test() {
  ...
  local out="/tmp/bitmoji-lab-$(date +%Y%m%d-%H%M%S).png"
...
show_menu() {
  ...
  cat <<EOF
${BOLD}${MAGENTA}+==================================================================+
|   Bitmoji Lab Android . management console (Windows $SHELL_ENV)${RESET}${BOLD}${MAGENTA}
